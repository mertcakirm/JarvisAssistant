...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    if output_path:
        p = Path(output_path)
        return p if p.is_absolute() else _get_base_dir() / p
    ext = ext_map.get((language or "python").lower(), ".py")
    return _get_base_dir() / f"jarvis_code{ext}"
...
def _write_action(description, language, output_path, player) -> str:
    if not description:
        return "Error: Missing description. If you already have the exact raw code and just want to save it to a file, please use the `file_controller` tool with action='write' and provide the code in the 'content' parameter."
    if player:
