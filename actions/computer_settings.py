...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            # Try PowerShell first as it's often more reliable for simple volume setting
            # This requires a small helper or a specific command
            subprocess.run(
                ["powershell", "-Command",
                 f"$obj = New-Object -ComObject WScript.Shell; "
                 f"for($i=0; $i -lt 50; $i++) {{ $obj.SendKeys([char]174) }}; " # Mute/Zero out
                 f"for($i=0; $i -lt {value//2}; $i++) {{ $obj.SendKeys([char]175) }}"], # Set to level
                capture_output=True
            )
            # Also try pycaw for precision if available
            try:
                import math
            ...
            print(f"[Settings] Windows volume set failed: {e}")
    elif _OS == "Darwin":
...
def pause_video():
    if _OS == "Darwin":
        # Global Media Play/Pause key code on Mac
        subprocess.run(["osascript", "-e", 'tell application "System Events" to key code 103'], capture_output=True)
        # Fallback to space if the above didn't work for focused window
...
def stop_media():
    if _OS == "Darwin":
        # 1. Global Media Play/Pause
        subprocess.run(["osascript", "-e", 'tell application "System Events" to key code 103'], capture_output=True)
        # 2. Specifically target Music/Spotify
        subprocess.run(["osascript", "-e", 'if application "Music" is running then tell application "Music" to pause'], capture_output=True)
        subprocess.run(["osascript", "-e", 'if application "Spotify" is running then tell application "Spotify" to pause'], capture_output=True)
        # 3. If a browser is open, 'k' or 'space' often works for YouTube
        pyautogui.press("k") 
    else:
        # For Windows/Linux, try global media stop and playpause
        try:
            pyautogui.press("mediastop")
        except:
            pass
        pyautogui.press("playpause")
...
def play_media():
    if _OS == "Darwin":
        ...
        subprocess.run(["osascript", "-e", 'tell application "System Events" to key code 103'], capture_output=True)
        # 2. Specifically target Music/Spotify
        subprocess.run(["osascript", "-e", 'if application "Music" is running then tell application "Music" to play'], capture_output=True)
        subprocess.run(["osascript", "-e", 'if application "Spotify" is running then tell application "Spotify" to play'], capture_output=True)
        # 3. Fallback to space for browsers
        pyautogui.press("space")
    else:
        pyautogui.press("playpause")
...
def computer_settings(
    ...
    if action == "close_app":
        app_to_close = str(value or "").strip()
        if not app_to_close:
            desc_lower = description.lower()
            for kw in ["close ", "kapat "]:
                if kw in desc_lower:
                    app_to_close = desc_lower.split(kw, 1)[1].strip()
                    break
            if not app_to_close and "kapat" in desc_lower:
                app_to_close = desc_lower.replace("kapat", "").strip()

        if app_to_close and app_to_close not in ["app", "uygulama", "window", "pencere", "uygulamayı", "uygulamayi", "sekme", "tab"]:
            try:
                if _OS == "Darwin":
                    subprocess.run(["osascript", "-e", f'quit app "{app_to_close}"'], capture_output=True)
                elif _OS == "Windows":
                    subprocess.run(["taskkill", "/IM", f"{app_to_close}.exe", "/F"], capture_output=True)
                else:
                    subprocess.run(["killall", app_to_close], capture_output=True)
                return f"Closed application: {app_to_close}"
            except Exception as e:
                return f"Failed to close {app_to_close}: {e}"
        else:
            close_app()
            return "Sent close application shortcut."

    if action == "close_window":
        close_window()
        return "Sent close window shortcut."
