...
def file_controller(
    ...
    try:
        if action == "list":
            ...
            from actions.desktop import organize_desktop
...
def empty_trash() -> str:
    ...
    try:
        if _OS == "Windows":
            ...
            result = SHELL32.SHEmptyRecycleBinW(None, None, 7) # 7 = No confirmation, no progress UI, no sound
            return "Trash emptied." if result == 0 else f"Could not empty trash, error code: {result}"
        elif _OS == "Darwin":
            import subprocess
            res = subprocess.run(["osascript", "-e", 'tell application "Finder" to empty trash'], capture_output=True, text=True)
            if res.returncode == 0:
                return "Trash emptied completely."
            else:
                return f"Could not empty trash: {res.stderr.strip()}"
        else: # Linux
            import shutil
            import subprocess
            if shutil.which("trash-empty"):
                res = subprocess.run(["trash-empty"], capture_output=True, text=True)
                return "Trash emptied." if res.returncode == 0 else f"Failed to empty trash: {res.stderr.strip()}"
            else:
                return "trash-cli is not installed. Run 'sudo apt install trash-cli' or equivalent."
    except Exception as e:
